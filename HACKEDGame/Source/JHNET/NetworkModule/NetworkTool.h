...
namespace NetworkTool {
	// Send with len(int)
	int Send(UObject* worldContext, SOCKET sock, const char* buf, int len, int flags = 0);
	int SendTo(UObject* worldContext, SOCKET sock, const char* buf, const int& len, sockaddr* addr, const int& addrLen);
